...
import_re = r"import \{[\s\S]*?\} from '@mui/material';\nimport \{[\s\S]*?\} from 'lucide-react';"
import re
content = re.sub(import_re, imports, content, 1)
...
"""
content = content.replace("export default function Attendance() {", helpers + "\nexport default function Attendance() {")
...
"""
content = content.replace("const [adminRecords, setAdminRecords] = useState([]);", "const [adminRecords, setAdminRecords] = useState([]);" + new_states)

# Fix fetchAdminRecords
old_fetch = r"const fetchAdminRecords = async \(\) => \{[\s\S]*?setAdminRecords\(response\.data\.results \|\| response\.data\);\n    \} catch \(e\) \{\n      console\.error\(e\);\n    \}\n  \};"
new_fetch = """const fetchAdminRecords = async () => {
    if (!isAdmin) return;
...
# Change the useEffect to listen to filters and page
old_effect = r"useEffect\(\(\) => \{\n    fetchHistory\(\);\n    fetchOvertimeRequests\(\);\n    if \(isAdmin\) \{\n      fetchAdminRecords\(\);\n    \}[\s\S]*?\}, \[isAdmin, selectedFirm\]\);"
...
# Now, replace the JSX Admin UI.
# In old_Attendance.jsx, the Admin UI starts at `{isAdmin ? (` and ends before `) : (` for the employee UI.
# Let's find it.
admin_ui = """{isAdmin ? (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
            <Typography variant="h5" sx={{ fontWeight: 800 }}>Team Attendance Records</Typography>
          </Box>
          ...
          <Card sx={{ mb: 3, p: 2, background: 'rgba(255,255,255,0.02)' }}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6} md={2}>
                <TextField fullWidth size="small" type="date" name="date" label="Exact Date" InputLabelProps={{ shrink: true }} value={filters.date} onChange={handleFilterChange} />
              </Grid>
              <Grid item xs={12} sm={6} md={2}>
                <TextField fullWidth size="small" type="date" name="startDate" label="Start Date" InputLabelProps={{ shrink: true }} value={filters.startDate} onChange={handleFilterChange} />
              </Grid>
              <Grid item xs={12} sm={6} md={2}>
                <TextField fullWidth size="small" type="date" name="endDate" label="End Date" InputLabelProps={{ shrink: true }} value={filters.endDate} onChange={handleFilterChange} />
              </Grid>
              <Grid item xs={12} sm={6} md={2}>
                <TextField fullWidth size="small" name="employee" label="Employee" value={filters.employee} onChange={handleFilterChange} />
              </Grid>
              <Grid item xs={12} sm={6} md={2}>
              ...
              <Grid item xs={12} sm={6} md={2}>
                <Button variant="outlined" color="error" fullWidth onClick={() => { setFilters({ date: '', startDate: '', endDate: '', employee: '', status: '', autoCheckout: '', overtimePending: '' }); setPage(1); }}>
                  Clear
                </Button>
              </Grid>
            </Grid>
          ...
          {adminLoading ? (
            ...
            Object.keys(groupedRecords).sort((a,b) => new Date(b) - new Date(a)).map(date => {
              ...
              return (
                <Accordion key={date} expanded={isDateExpanded} onChange={() => toggleDate(date)} sx={{ mb: 2, bgcolor: 'background.paper', borderRadius: '8px !important', '&:before': { display: 'none' } }}>
                  <AccordionSummary expandIcon={<ChevronDown />}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 4, width: '100%', flexWrap: 'wrap' }}>
                      <Typography variant="h6" sx={{ fontWeight: 800, color: 'primary.main', minWidth: 120 }}>{formatDate(date)}</Typography>
                  ...
                  <AccordionDetails sx={{ bgcolor: 'rgba(0,0,0,0.1)', p: 2 }}>
                    {dateRecords.map(rec => {
                      ...
                      return (
                        <Accordion key={empKey} expanded={isEmpExpanded} onChange={() => toggleEmployee(date, rec.id)} sx={{ mb: 1, bgcolor: 'background.default', borderRadius: '6px !important', '&:before': { display: 'none' } }}>
                          <AccordionSummary expandIcon={<ChevronDown />}>
                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', pr: 2, flexWrap: 'wrap', gap: 2 }}>
                              <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                                <Typography variant="subtitle1" sx={{ fontWeight: 800, color: 'secondary.main', textTransform: 'uppercase' }}>{rec.employee_name}</Typography>
                                <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 600 }}>@{rec.employee_username || rec.employee_name.toLowerCase().replace(' ', '_')}</Typography>
                              </Box>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                                <Typography variant="body2" sx={{ fontWeight: 600 }}>Total: {parseFloat(rec.daily_total_hours).toFixed(2)}h</Typography>
                                <Typography variant="body2" sx={{ fontWeight: 600 }}>Break: {parseFloat(rec.daily_break_hours).toFixed(2)}h</Typography>
                              ...
                              <Table size="small" sx={{ minWidth: 800 }}>
                                ...
                                <TableBody>
                                  {(!rec.sessions || rec.sessions.length === 0) ? (
                                    <TableRow><TableCell colSpan={7} align="center" sx={{ py: 3, color: 'text.secondary' }}>No sessions recorded.</TableCell></TableRow>
                                    ...
                                    <TableRow key={sess.id} hover>
                                      ...
                                      <TableCell>
                                        <Box sx={{ display: 'flex', gap: 1 }}>
                                          <Tooltip title="Check In Photo"><Box><PhotoPreview url={sess.captured_image} label="In" onClick={openPhoto} /></Box></Tooltip>
                                          <Tooltip title="Check Out Photo"><Box><PhotoPreview url={sess.check_out_captured_image} label="Out" onClick={openPhoto} /></Box></Tooltip>
                                        </Box>
                                      </TableCell>
                                        ...
                                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                                          <Tooltip title={sess.check_in_address || 'Unknown In Location'}>
                                            <Typography variant="caption" sx={{ display: 'flex', alignItems: 'center', gap: 0.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                              <MapPin size={12} color="#00f5d4" style={{flexShrink: 0}} /> {sess.check_in_address || '---'}
                                            </Typography>
                                          </Tooltip>
                                          <Tooltip title={sess.check_out_address || 'Unknown Out Location'}>
                                            <Typography variant="caption" sx={{ display: 'flex', alignItems: 'center', gap: 0.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                              <MapPin size={12} color="#ff9f43" style={{flexShrink: 0}} /> {sess.check_out_address || '---'}
                                            </Typography>
                                          </Tooltip>
                                        </Box>
                                      </TableCell>
                                      <TableCell align="right">
                                        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
                                          <Button size="small" variant="outlined" color="primary" onClick={() => { setSelectedSessionForEdit(sess); setEditModalOpen(true); }}>Edit</Button>
                                          <Button size="small" variant="outlined" color="error" onClick={() => handleDeleteSession(sess.id)}>Delete</Button>
                                        </Box>
                                      </TableCell>
                                    </TableRow>
                                  ))}
...
# Replace `{isAdmin ? (...) : (`
old_admin_block = re.search(r"\{isAdmin \? \([\s\S]*?\) : \(", content)
if old_admin_block:
    content = content.replace(old_admin_block.group(0), admin_ui)

# Add the photo viewer dialog at the end
photo_dialog = """
