...
if not admin_user:
    admin_user = User.objects.create(username="test_superadmin", role="SUPER_ADMIN", company=company)
...
Attendance.objects.filter(employee=emp_user, date=timezone.now().date() - timedelta(days=2)).delete()
att = Attendance.objects.create(employee=emp_user, date=timezone.now().date() - timedelta(days=2), status='LATE', shift=shift)
...
response = client.patch('/api/v1/attendance/records/edit-session/', {
    'session_id': sess.id,
...
if response.status_code == 200:
    ...
    for log in logs:
        print(f"Log: Reason={log.reason}, Old={log.old_value}, New={log.new_value}")
else:
